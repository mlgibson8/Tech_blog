...
.then(dbPostData => {
    ...
    for (let i = 0; i < dbPostData.dataValues.comments.length; i++) {
        ...
        let commentDate = dbPostData.dataValues.comments[i].created_at;
        let user_id = dbPostData.dataValues.comments[i].user_id;
        let comment_id = dbPostData.dataValues.comments[i].id;
        post.comments.push({
            user: username,
            userId: user_id,
            text: commentText,
            ...
            userComment: username === req.session.username,
        });
    }
